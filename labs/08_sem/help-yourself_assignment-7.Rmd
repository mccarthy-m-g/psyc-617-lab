...
Last thing in the course! Hopefully this drives home the point that you shouldn't take a cookbook approach to statistics. You may have been taught that way, but you do not need to think that way. We're going to use the very first data set you did a student's t-test on in PSYC 615 for this one, but we're now going to do the t-test as a structural equation model. This answer about using SEM to run one-way and two-way ANOVAs on Cross Validated makes a good reference for the various ways to do this <https://stats.stackexchange.com/a/453558/337979>.
...
Here's the student's t-test as a structural equation model. There are two ways to go about this. First, we could simply do a regression with lavaan.
...
Second, we could test for measurement invariance between the two groups. Here it's less obvious since the estimates and test statistics aren't the same, but we're answering a similar question.
